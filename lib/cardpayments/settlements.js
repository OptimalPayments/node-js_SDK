...
var order = require("../hostedpayment/order");
function settlements(resp) {
	if (resp) {
		...
		if (resp.order) {
			this.order = new order(resp.order);
		}
...
settlements.prototype.setOrder = function(order) {
	this.order = order;
};

settlements.prototype.getOrder = function() {
	return this.order;
};
