 ...
 */
var CardServiceHandler = function(optimalApiClient) {
...
CardServiceHandler.prototype.authorize = function(auth, responseCallBack) {
	try {
		var clientObj = this._optimalApiClient;
		if (typeof (responseCallBack) === "function") {
			var requestObj = new optPayRequest(prepareURI(AUTH_PATH, clientObj),
					constants.POST);
	...
	try {
		...
		if (typeof (responseCallBack) === "function") {
			if (auth && auth.id) {
				...
				var requestObj = new optPayRequest(prepareURI(AUTH_PATH + authId,
	...
	try {
		...
		if (typeof (responseCallBack) === "function") {
			if (settle && settle.id) {
				...
				var requestObj = new optPayRequest(prepareURI(SETTLEMENTS + "/"
	...
	try {
		...
		if (typeof (responseCallBack) === "function") {
			if (refund && refund.id) {
				...
				var requestObj = new optPayRequest(prepareURI(REFUNDS + "/" + refundId,
						clientObj), constants.PUT);
				clientObj.processRequest(requestObj, refund, function(error, response) {
					response = response ? new clientObj.Refund(response) : response;
					responseCallBack(error, response);
	...
	try {
		...
		if (typeof (responseCallBack) === "function") {
			if (auth && auth.id) {
				var authId = auth.id;
				delete auth.id;
				var optPayRequestObj = new optPayRequest(prepareURI(AUTH_PATH + authId,
						clientObj), constants.GET);
				clientObj.processRequest(optPayRequestObj, null, function(error,
						response) {
					response = response ? new clientObj.Authorization(response)
							: response;
					responseCallBack(error, response);
	...
	try {
		...
		if (typeof (responseCallBack) === "function") {
			if (authReversal && authReversal.id) {
				...
				var requestObj = new optPayRequest(prepareURI(REVERSAUTH + authId,
						clientObj), constants.GET);
...
CardServiceHandler.prototype.verify = function(verification, responseCallBack) {
	try {
		var clientObj = this._optimalApiClient;
		if (typeof (responseCallBack) === "function") {
				var requestObj = new optPayRequest(prepareURI(VERIFICATION,
	...
	try {
		...
		if (typeof (responseCallBack) === "function") {
			if (verification && verification.id) {
				...
				var requestObj = new optPayRequest(prepareURI(VERIFICATION + "/" + verificationId,
						clientObj), constants.GET);
				clientObj.processRequest(requestObj, null, function(error, response) {
					response = response ? new clientObj.Verification(response) : response;
					responseCallBack(error, response);
